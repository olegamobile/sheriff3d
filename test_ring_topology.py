...
def get_closed_ring(x, n_half=32):
    # n_half points on hull (keel to sheer) + deck points
    b = max(0.001, float(cs_beam(x)))
    kz = float(cs_keel(x))
    sz = float(cs_sheer(x))
    t = x / 6000.0
    ...
    if x >= 4000:
        ...
        pts_y = [b, cab_w + 30, cab_w, cab_w * 0.88, roof_step_w, 0.0, -roof_step_w, -cab_w * 0.88, -cab_w, -cab_w - 30, -b]
        pts_z = [sz, sz + 15, sz + 20, cab_roof_z, cab_roof_z + 20, cab_roof_z + 30, cab_roof_z + 20, cab_roof_z, sz + 20, sz + 15, sz]
    ...
    # 3. Port hull (from -b at sheer to 0 at keel)
    theta_port = np.linspace(np.pi/2, 0, n_half)
    y_hull_port = -b * (np.sin(theta_port) ** p)
    z_hull_port = kz + (sz - kz) * (1.0 - np.cos(theta_port) ** q)
    
    # Assemble full ring:
    # Starboard hull (exclude sheer to avoid duplicate): y_hull_stbd[:-1], z_hull_stbd[:-1]
    # Deck: y_deck, z_deck
    # Port hull (exclude sheer and keel): y_hull_port[1:-1], z_hull_port[1:-1]
...
# Test building full mesh from rings
nx = 51
x_vals = np.linspace(0, 6000, nx)
rings = [get_closed_ring(x) for x in x_vals]
m_pts = len(rings[0])
print(f"Ring resolution: {m_pts} points per station, {nx} stations")
...
# Cap transom at ix = 0 (using fan triangulation from center)
transom_pts = rings[0]
center_pt = np.array([[0.0, 0.0, np.mean(transom_pts[:, 2])]])
c_idx = len(all_verts)
all_verts = np.vstack([all_verts, center_pt])
for im in range(m_pts):
    im_next = (im + 1) % m_pts
    # Winding facing outward (-x)
    all_faces.append([c_idx, im_next, im])
    
# Cap bow stem at ix = nx - 1
bow_pts = rings[-1]
...
hull_mesh = trimesh.Trimesh(vertices=all_verts, faces=np.array(all_faces))
hull_mesh.process()
trimesh.repair.fix_normals(hull_mesh)
print(f"HULL SOLID: Watertight={hull_mesh.is_watertight}, Volume={hull_mesh.volume:.1f} mm³")
