...
4. **Кормовая переборка** — одна плоскость, наклонённая вперёд на 30° (параллельно заднему ребру окна), спускается на 50 мм ниже уровня банок до порожка (Z 450, банки Z 500) и поднимается до крыши. Ниже уровня борта по бортам от неё рубка выступает назад вдоль комингсов «крыльями» (R 450), которые заканчиваются **вогнутыми четвертями окружности** (центр дуги в кокпите), касательными к комингсу и к переборке: каждая банка упирается в полукруглую стенку. Верх крыльев — на уровне планширя, заподлицо с ним. **Выше уровня борта** переборка — одна плоскость во всю ширину с острыми углами. В переборке **большой вход в рубку** — проём 700 мм от порожка почти до крыши, выполненный карманом глубиной 200 мм.
...
## 🎨 Интеграция с Blender (Blender MCP & `.blend`)

В репозиторий добавлен готовый проект для Blender:
- **`Jouet_Sheriff_600.blend`** — Сцена Blender 5.2 со структурированными коллекциями:
  - `Jouet_Sheriff_1to45` (разбитая на детали модель: палуба, киль, руль, рангоут, кильблок).
  - Настроенные PBR-материалы (гелькоут корпуса, тиковые банки, тонированный акрил, чугунный киль, алюминиевый рангоут).
  - Студийный свет (Key, Fill, Rim) и настроенная камера.
- **`build_blender_project.py`** — Автоматический скрипт сборки сцены в Blender.
- **Blender MCP:** настроен аддон `blender_mcp` и сервер для прямого взаимодействия AI-агентов с 3D-вьюпортом Blender.
